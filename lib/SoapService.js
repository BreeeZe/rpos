...
var SoapService = (function () {
    ...
    SoapService.prototype.start = function () {
        var _this = this;
        this.starting();
        utils_1.utils.log.info("Starting webserver on port: %s", this.config.ServicePort);
        this.webserver.listen(this.config.ServicePort);
        utils_1.utils.log.info("Binding %s to %s", this.constructor.name, this.serviceOptions.path);
        ...
        this.serviceInstance.on("request", function (request, methodName) {
            utils_1.utils.log.debug('%s received request %s', _this.constructor.name, methodName);
        });
        this.serviceInstance.log = function (type, data) {
            if (_this.config.logSoapCalls)
                utils_1.utils.log.debug('%s - Calltype : %s, Data : %s', _this.constructor.name, type, data);
        };
    };
