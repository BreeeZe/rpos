      ...
      for (var uc in controls) {
        ...
        if (p.hasSet()) {
          ...
          for (var s = 0; s < set.length; s++) {
            var o = set[s];
            html += ['<option value="', o.value, '" ', o.value == p.get() ? 'selected="selected "' : '', '>', o.lookup, '</option>'].join('');
          }
          html += '</select></td></tr>';

        } else if(p.type() == "Boolean") {
          html += ['<tr><td><span class="label">', uc, '</span></td><td><input type="hidden" name="', propname, '.', uc, '" value="false" /><input type="checkbox" name="', propname, '.', uc, '" value="true"', p.get() ? 'checked="checked"' : '', '" /></td><tr>'].join('');
        } else {
          html += ['<tr><td><span class="label">', uc, '</span></td><td><input type="text" name="', propname, '.', uc, '" value="', p.get(), '" /></td><tr>'].join('');
        }
      }
      return html;
    }
