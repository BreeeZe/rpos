...
Camera.prototype.startAll = function () {
  ...
  if (!utils.isWin()) /* Don't do is on development machine */ {
    this.startRtsp();
    this.startGst();
  }
};
...
Camera.prototype.startVideo = function (pipe) {
  ...
  if (this.settings.quality) {
    vidParameters.push("-qp");
    vidParameters.push(10 + ((this.settings.quality - 10) * -3));
  } else if (this.settings.bitrate) {
    vidParameters.push("-b");
    vidParameters.push(this.settings.bitrate * 1000);
  }
  
  if (this.settings.exposure) {
    vidParameters.push("-ex");
    vidParameters.push(this.settings.exposure);
  }

  if (this.settings.profile) {
    vidParameters.push("-pf");
    vidParameters.push(this.settings.profile);
  }
  
  if (this.settings.hf)
    vidParameters.push("-hf");
  
  if (this.settings.vf)
    vidParameters.push("-vf");
  
  if (this.settings.drc > 0) {
    var drc = this.settings.drc;
    vidParameters.push("-drc");
    vidParameters.push(drc == 1 ? "low" : drc == 2 ? "medium" : drc == 3 ? "high" : "off");
  }

  if (!utils.isWin()) /* Don't do is on development machine */ {
    this.raspivid = spawn("raspivid", vidParameters);
...
Camera.prototype.stopVideo = function () {
  if (this.raspivid && !utils.isWin()) /* Don't do is on development machine */ {
