...
SimpleTypeElement.prototype.description = function (definitions) {
    ...
    for (var i = 0, child; child = children[i]; i++) {
        if (child instanceof RestrictionElement)
            return new String(this.$name + "|" + child.description());
    }
    return {};
};
...
WSDL.prototype.objectToRpcXML = function (name, params, namespace, xmlns) {
    ...
    for (var key in params) {
        if (key !== nsAttrName) {
            ...
            value = (typeof value === 'object') ? this.objectToXML(value, key, namespace, xmlns) : xmlEscape(value);
            if (value) {
                parts.push(['<', key, '>'].join(''));
                parts.push(value);
                parts.push(['</', key, '>'].join(''));
...
WSDL.prototype.objectToXML = function (obj, name, namespace, xmlns, first, xmlnsAttr, parameterTypeObject, ancestorXmlns) {
    ...
    if (Array.isArray(obj)) {
        for (var i = 0, item; item = obj[i]; i++) {
            ...
            var v = self.objectToXML(item, name, namespace, xmlns, false, null, parameterTypeObject, ancXmlns);
            if (v) {
        ...
        for (name in obj) {
                ...
                if (self.definitions.schemas) {
                    if (schema) {
                        ...
                        if (childParameterTypeObject && childParameterTypeObject.$type && (childParameterTypeObject.$type.indexOf('xsd') === -1)) {
                            ...
                            for (var ignoredNamespacesIndex = 0, ignoredNamespacesLength = this.options.ignoredNamespaces.length; ignoredNamespacesIndex < ignoredNamespacesLength; ignoredNamespacesIndex++) {
                                if (this.options.ignoredNamespaces[ignoredNamespacesIndex] === childNamespace) {
            ...
            if (!Array.isArray(child)) {
                parts.push(['<', ns, name, attr, xmlnsAttrib, (value ? '>' : '/>')].join(''));
            }
